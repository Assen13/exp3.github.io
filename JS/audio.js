...
var bgContainer = document.querySelector('.bg-container'); // 获取背景容器
//列表按钮
...
//后台音乐列表
let musicData = [['竈門炭治郎のうた', '25216950236黄春山'], ['하루하루', '25216950236黄春山'], ['紅蓮華', '25216950236黄春山'], ['枫', '25216950236黄春山']]
var muscics = [['竈門炭治郎のうた', '25216950236黄春山'],['하루하루', '25216950236黄春山'] , ['紅蓮華', '25216950236黄春山'], ['枫', '25216950236黄春山']]

// 初始化音乐功能
function initMusic() {
    audio.src = `mp3/music${musicId}.mp3`;// 找到对应MP3文件
    audio.load();// 加载音乐
    //音乐加载完成后，更新歌名、歌手、背景图、唱片图。当歌曲信息被加载完成时,或者用onloadedmetadata事件也可以
    audio.ondurationchange = function () {
        musicTitle.innerText = muscics[musicId][0];// 显示歌名
        author.innerText = muscics[musicId][1]; // 显示歌手（含学号）
        recordImg.style.backgroundImage = `url('img/record${musicId}.jpg')`;// 换唱片图
        bgContainer.style.backgroundImage = `url('img/bg${musicId}.png')`; //// 换背景图
        totalTime.innerText = transTime(audio.duration);// 显示总时长
        // 重置进度条
        audio.currentTime = 0;
        updateProgress();
...
pause.addEventListener("click", function (e) {
    if (audio.paused) {// 如果当前是暂停状态
        audio.play();// 播放音乐
        rotateRecord();// 启动唱片旋转
        pause.classList.remove("icon-play");// 换“暂停”图标
        pause.classList.add("icon-pause");
    } else {// 如果当前是播放状态
        audio.pause();// 暂停音乐
        rotateRecordStop();// 停止唱片旋转
        pause.classList.remove("icon-pause");// 换“播放”图标
...
function rotateRecord() {
    recordImg.style.animationPlayState = "running";// 开始旋转
}
function rotateRecordStop() {
    recordImg.style.animationPlayState = "paused";// 停止旋转
}
...
skipForward.addEventListener('click', function () {
    musicId = musicId - 1;// 序号减1
    if (musicId < 0) { // 如果到第一首了，跳回最后一首
...
skipBackward.addEventListener('click', function () {
    musicId = musicId + 1;
    if (musicId == muscics.length) {// 如果到最后一首了，跳回第一首
        musicId = 0;
    }
    initAndPlay();// 重新加载并播放
    pause.classList.remove("icon-play");
